...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = lora.BiDoRALinear(config.hidden_size, self.all_head_size, config.lora_r,
                                       lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout)

        self.key = nn.Linear(config.hidden_size, self.all_head_size)

        self.value = lora.BiDoRALinear(config.hidden_size, self.all_head_size, config.lora_r,
                                       lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout)
    ...
    def forward(
        ...
        if self.config.lora_type == 'adapter' or self.config.lora_type == 'ft':
            ...
            mixed_query_layer = self.query(hidden_states, alphas[0] if alphas is not None else None)
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if self.config.lora_type == 'adapter' or self.config.lora_type == 'ft':
                ...
                value_layer = self.transpose_for_scores(
                    self.value(encoder_hidden_states, alphas[1] if alphas is not None else None))
            attention_mask = encoder_attention_mask
            ...
            if self.config.lora_type == 'adapter' or self.config.lora_type == 'ft':
                ...
                value_layer = self.transpose_for_scores(
                    self.value(hidden_states, alphas[1] if alphas is not None else None))
