    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            output_states = layer_module(
                alphas[i] if alphas is not None else None,
                next_kv,
                attention_mask,
                output_attentions,
                query_states=query_states,
...
class DisentangledSelfAttention(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.query_proj = lora.BiDoRALinear(config.hidden_size, self.all_head_size, r=config.lora_r,
                                            lora_alpha=config.lora_alpha, merge_weights=False,
                                            lora_dropout=config.lora_dropout)

        self.key_proj = nn.Linear(config.hidden_size, self.all_head_size, bias=True)

        self.value_proj = lora.BiDoRALinear(config.hidden_size, self.all_head_size, r=config.lora_r,
                                            lora_alpha=config.lora_alpha, merge_weights=False,
                                            lora_dropout=config.lora_dropout)
    ...
    def forward(
        ...
        if query_states is None:
            query_states = hidden_states
        if self.config.lora_type == 'adapter' or self.config.lora_type == 'ft':
            q = self.query_proj(query_states)
        else:
            q = self.query_proj(query_states, alphas[0] if alphas is not None else None)
        k = self.key_proj(hidden_states)
        if self.config.lora_type == 'adapter' or self.config.lora_type == 'ft':
            v = self.value_proj(hidden_states)
        else:
            v = self.value_proj(hidden_states, alphas[1] if alphas is not None else None)
    ...
    def manually_gather(self, input, index):
        ...
        index = index + torch.arange(start=0, end=index.size(0) * input.shape[-1], step=input.shape[-1],
                                     device=index.device).view(-1, 1)
        return torch.index_select(input.view(input.shape[0], -1), dim=-1, index=index.view(-1)).view(input.shape[0],
                                                                                                     index.shape[0],
                                                                                                     index.shape[1])
    ...
    def forward(
        ...
        if self.masking_prob > 0:
            random_indices = torch.bernoulli(
                torch.full(input_ids.shape, self.masking_prob, device=input_ids.device)).bool()
        ...
        if labels is not None:
            if labels.dim() == 1 or labels.size(-1) == 1:
                ...
                if label_index.size(0) > 0:
                    ...
                    p1_m_loss = F.kl_div(F.log_softmax(p1_logits, dim=-1), F.softmax(m_logits, dim=-1),
                                         reduction='mean')
                    p2_m_loss = F.kl_div(F.log_softmax(p2_logits, dim=-1), F.softmax(m_logits, dim=-1),
                                         reduction='mean')
                else:
        ...
        if labels is not None:
            if labels.dim() == 1 or labels.size(-1) == 1:
                ...
                if label_index.size(0) > 0:
                    ...
                    p1_loss = loss_fct(p1_logits.view(-1, self.num_labels).float(), labels.view(-1))
                    p2_loss = loss_fct(p2_logits.view(-1, self.num_labels).float(), labels.view(-1))
                    p12_loss = F.kl_div(F.log_softmax(p1_logits, dim=-1), F.softmax(p2_logits, dim=-1),
                                        reduction='mean')
                    p21_loss = F.kl_div(F.log_softmax(p2_logits, dim=-1), F.softmax(p1_logits, dim=-1),
                                        reduction='mean')
                else:
                    p1_loss = torch.tensor(0).to(p1_logits)
                    p2_loss = torch.tensor(0).to(p2_logits)
