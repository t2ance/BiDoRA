...
def add_optimizer_params(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--arch_lr', default=0.00001, type=float, help='architecture learning rate')
    parser.add_argument('--weight_decay', default=0.01, type=float, help='weight decay rate')
    parser.add_argument('--arch_weight_decay', default=0.01, type=float, help='architecture weight decay rate')
    ...
    parser.add_argument('--warmup_step', type=int, default=0, help='warmup step')
...
def create_grouped_lora_parameters(model, no_decay_bias):  # args):
    if not no_decay_bias:
        # only train the classifier head or lora_A and lora_B
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in model.named_parameters() if
                           "lora_" in n or "classifier" in n or 'adapter' in n],
                # if not any(nd in n for nd in no_decay)],
            }]
        name = {n: p.numel() for n, p in model.named_parameters() if
                "lora_" in n or "classifier" in n or 'adapter' in n}
        all_name = {n: p.numel() for n, p in model.named_parameters()}
        print(
            f'#parameters peft: {sum([p.numel() for n, p in model.named_parameters() if "lora_" in n or "adapter" in n])}')
        print(f'create_grouped_lora_parameters: {name}')
        print(f'[ALL] create_grouped_lora_parameters: {all_name}')
    else:
        no_decay = ["bias", "layer_norm.weight"]
        ...
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in model.named_parameters() if
                           not any(nd in n for nd in no_decay) and "lora_" in n],
            },
            {
                "params": [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)],
                "weight_decay": 0.0,
...
def create_arch_adam_optimizer_from_args(model, args, grouped_parameters=None):
    if grouped_parameters is None:
        grouped_parameters = create_grouped_parameters(model, args.no_decay_bias)
...
def create_lora_adam_optimizer_from_args(model, args, grouped_parameters=None):
    if grouped_parameters is None:
        grouped_parameters = create_grouped_lora_parameters(model, args.no_decay_bias)
...
def create_optimizer_scheduler(optimizer, args, is_arch: bool = False):
    if is_arch:
        max_step = int(args.max_step / args.unroll_step)
        warmup_step = int(args.warmup_step / args.unroll_step)
    else:
    ...
    if args.scheduler == 'cosine':
        scheduler = CosineAnnealingWarmupRestarts(
            ...
            max_steps=max_step, alpha=0
        )
    elif args.scheduler == 'linear':
        scheduler = get_linear_schedule_with_warmup(
            optimizer, warmup_step, max_step, last_epoch=-1
        )
        ...
        max_step = args.i_steps[-1]
        print('max_step is rest to', max_step)
        scheduler = CyclicScheduler(
            optimizer, interval_steps=args.i_steps, interval_lrs=args.i_lrs
        )
    elif args.scheduler == 'constant':
        scheduler = get_constant_schedule_with_warmup(
            optimizer, warmup_step, args.max_step, last_epoch=-1
        )
    else:
